...
<head>
    ...
    <title>Ordem de Pagamento nº {{ $op->numero }} — Instrumento {{ $instrumento->numero }}</title>
    <style>
...
<div class="page">
    ...
    <div class="doc">
        {!! $op->conteudo !!}
        @include('processos._carimbo', ['peca' => $op, 'qrValidacao' => $qrValidacao])
    </div>

    <p class="rodape">Instrumento nº {{ $instrumento->numero }} · Ordem de Pagamento nº {{ $op->numero }}</p>
</div>
</body>
</html>
