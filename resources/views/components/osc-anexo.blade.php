...
<div class="mt-3">
    ...
    <input id="{{ $campo }}" name="{{ $campo }}" type="file" accept=".pdf,.jpg,.jpeg,.png"
           class="mt-1 block w-full text-sm text-gray-600 file:mr-3 file:py-1 file:px-3 file:rounded file:border-0 file:text-xs file:bg-brand-50 file:text-brand-700 hover:file:bg-brand-100">
    @if($osc?->{$campo})
        <p class="text-xs text-gray-500 mt-1">
            Atual:
            <a href="{{ route('oscs.anexo', [$osc, $campo]) }}" class="text-brand-600 hover:underline" target="_blank">
