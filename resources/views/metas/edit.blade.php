<x-app-layout>
    <x-slot name="header">
        ...
        <h2 class="text-xl font-semibold text-gray-800 mt-0.5">Editar Meta {{ $meta->numero }}</h2>
    </x-slot>

    <div class="py-8">
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white shadow rounded-lg p-6">
                <form action="{{ route('propostas.metas.update', [$proposta, $meta]) }}" method="POST" class="space-y-4">
                    @csrf
                    @method('PUT')
                    @include('metas._form', ['meta' => $meta])
